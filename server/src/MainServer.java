...
public class MainServer {
    ...
    private static void binding() throws IOException {
        ...
        try {
            GetAndSendMessage.port = Integer.parseInt(new Scanner(System.in).nextLine());
        ...
        try {
            GetAndSendMessage.server = DatagramChannel.open();
        } catch (IOException e) {
            e.printStackTrace();
        }
        InetSocketAddress iAdd = new InetSocketAddress("localhost", GetAndSendMessage.port);
        try {
            GetAndSendMessage.server.bind(iAdd);
